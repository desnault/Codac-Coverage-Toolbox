 ...
 * @brief Example of computing the covered area using SepDynPieProj with a binary perception sensor.
 *
 ...
 * - SepDynPieProj internally handles angle wrap-around using modulo2pi to ensure the FOV remains in [-π, π].
 *
...
// Binary sensor detection range (scalar)
double detection_range = 1.0;

// Sensor field-of-view in the robot frame (delta interval)
// Centered at 0, inflated by 45 degrees (pi/4)
...
int main()
{
    ...
    // Create the SepDynPieProj separator/projector
    // This combines SepDynPie separator with SepProj projection along the trajectory
    SepDynPieProj my_sep(my_tube, detection_range, fov, t_proj, eps_proj, true);
    ...
    for (double t : detection_time)
    {
        ...
        // Draw the directional detection area (pie) in robot frame
